...
var definition = {
  ...
  handlers: {
    terminate: function (self) {
      var count = ++self._state.termination.calls
      var message
      if (self._settings.termination.throwOnMultipleTerminations && count > 1) {
        message = 'VoxEngine.terminate has been called ' + count + ' times, ' +
          'which is forbidden by settings'
        throw new Error(message)
      }
      if (self !== this && self._settings.termination.throwOnLostContext) {
    ...
    removeEventListener: function (self, event, handler) {
      ...
      return index > -1 ? null : listeners.splice(index, 1)[0]
    },
